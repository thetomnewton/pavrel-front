...
const { currentWorkspaceTeams, updateTeam, updateTeamUserRole } = useTeams()
...
const iAmTeamAdmin = computed(() => team.value?.users.find(({ id }) => id === user.value.id)?.pivot.role === 'admin')
...
<template>
  <div class="mx-auto max-w-4xl px-6 py-10 lg:px-8" v-if="team && newTeam">
    ...
    <div class="mb-10 space-y-6 border-b border-slate-200 pb-10 dark:border-zinc-700 md:flex md:space-x-6 md:space-y-0">
      <div class="md:w-1/3">
      ...
      <div class="md:w-2/3">
        <form @submit.prevent="attemptTeamUpdate" class="max-w-[350px] space-y-6">
          ...
          <div>
            <Button :disabled="!newTeam.name || !newTeam.slug || slugAlreadyExists || teamUpdateStatus === 'updating'">
              Save
            </Button>
          </div>
        </form>
